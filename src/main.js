...
import {canvasPosToLatLng, colorpalette, debugLog, getDebugLoggingEnabled, saveDebugLoggingEnabled} from './utils.js';
...
function flyToLatLng(lat, lng, zoom = 16) {
    console.log("bmmap", unsafeWindow.bmmap);
...
for (let color of colorpalette) {
    palette[color["id"]] = color["rgb"]
}

// поиск мапы, хранящей сраные пиксели
function isSpotEntry(key, value) {
    if (typeof key !== "string") return false;
    if (!key.startsWith("t=(") || !key.includes("p=(") || !key.includes("s=")) {
        return false;
    }
    ...
    return (
        ...
        typeof value.colorIdx === "number" &&
        typeof value.color === "object"
    );
}

function patchMapColors(myMap) {
    const groups = new Map();

    for (const [key, entry] of myMap) {
        const [tileX, tileY] = entry["tile"];
    ...
    for (const [groupKey, items] of groups) {
        const [tileX, tileY] = groupKey.split(",").map(Number);

        const colorIdxList = items.map(i => i.entry["colorIdx"]);

        const pixelList = [];
        for (const i of items) {
            pixelList.push(i.entry["pixel"][0], i.entry["pixel"][1]);
        }

        const patchedColors = templateManager.getPixelsColorAt(
            tileX,
            tileY,
            colorIdxList,
            pixelList
        ...
        for (let i = 0; i < items.length; i++) {
            const [r, g, b] = palette[patchedColors[i]];
        ...
        if (thisArg && thisArg["__click"] === this) {
            ...
            if (shit.includes("fn.paint") && shit.includes("pg(kr)") && shit.includes("[...s.values()]")) {
                for (const aboba of maps) { // СУУУКААА ПОЧЕМУ НЕ IN А OF
                    ...
                    if (found) {
                        patchMapColors(aboba, (tileX, tileY, colors, pixels) => {
...
inject(() => {
    ...
    // Ноду больше нельзя получить таким способом, либо я немощь. Но тем не менее

    // Observer to wait for the map to be ready and set window.bmmap
    // const observer = new MutationObserver(mutations => {
        // console.log("nodes0", document.querySelector("div.absolute.bottom-3.right-3.z-30").childNodes[0]);
        // try {
...
async function attemptEmergencyRecovery() {
    ...
    try {
        ...
        if (typeof GM !== 'undefined' && GM.deleteValue) {
            ...
            for (let i = 0; i < 50; i++) {
                try {
                    await GM.deleteValue(`bmTemplates_part_${i}`);
                } catch (_) {
                }
            }
        }
...
function buildOverlayMain() {
    ...
    overlayMain.addDiv({'id': 'bm-overlay', 'style': 'top: 10px; right: 75px;'})
        ...
        .addButton({
                'id': 'bm-button-coords',
                'innerHTML': icons.pointerIcon + 'Detect',
        ...
        .addButton({
            ...
            innerHTML: (isTileRefreshPaused() ? icons.playIcon : icons.pauseIcon) + (isTileRefreshPaused() ? 'Resume' : 'Pause')
        }, (instance, button) => {
        ...
        .addButton({
            ...
            innerHTML: icons.colorFilterIcon + 'Color Filter'
        }, (instance, button) => {
            button.onclick = () => {
        ...
        .addButton({
            ...
            'className': 'bm-help',
            innerHTML: icons.clearStorageIcon,
        ...
        .buildElement()
        .addDiv({'style': 'position: absolute; left: 0; bottom: 2px; text-align: left; padding: 0; pointer-events: auto; user-select: text; line-height: 12px;'}).addSmall({
        'textContent': `Made by SwingTheVine | Fork Seris0 | v${version}`,
    ...
    `;
      ...
      `;
        ...
        filterSelect.addEventListener('change', () => {
            try {
                saveTemplateColorSort(filterSelect.value);
            } catch {
            }
            applyFilter(filterSelect.value);
        });
...
function buildCrosshairSettingsOverlay() {
  ...
  `;
        ...
        const updateMobileStateWrapped = () => {
            oldUpdateMobile();
            applyMobileVisual();
        };
  ...
  `;
        ...
        applyButton.onclick = async () => {
            ...
            try {
                // Save all settings
                debugLog('Applying crosshair settings:', {
                    color: tempColor,
                    borders: tempBorderEnabled,
                    miniTracker: tempMiniTrackerEnabled,
                    topBar: tempTopBarEnabled,
                    collapse: tempCollapseMinEnabled,
                    mobile: tempMobileMode,
                    showLeftOnColor: tempShowLeftOnColor,
                    navigation: tempNavigationMethod,
                    debug: tempDebugEnabled,
                    smartCache: tempCacheEnabled
                });
